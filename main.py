...
def run_statistical_suite(dataset_name='ml-100k', seeds=None):
    ...
    # ⬅️ REVERTED to vibe_only internally!
    architectures = [
        "lgcn_oracle",
        "lgcn_baseline",          
        "lgcn_tf_idf_raw",
    ...
    for idx, seed in enumerate(seeds):
        ...
        for strategy in ["raw", "vibe_only", "combination"]:
            ...
            tfidf_res = std_exp.run_evaluation(modifier_func=tfidf_mod, name=f"TF-IDF ({display_name})")
            results[f"lgcn_tf_idf_{strategy}"]['hit'].append(tfidf_res['hit@10'])
            results[f"lgcn_tf_idf_{strategy}"]['ndcg'].append(tfidf_res['ndcg@10'])
    ...
    print("="*125)
    print(f"{'Model Architecture':<40} | {'Hit@10 (Mean ± SD)':<25} | {'NDCG@10 (Mean ± SD)':<25} | {'Method'}")
    print("-" * 125)
    
    def print_row(key, name_override, note):
        h_m, h_s = calc_stats(results[key]['hit'])
        n_m, n_s = calc_stats(results[key]['ndcg'])
        print(f"{name_override:<40} | {h_m:.4f} ± {h_s:.4f}     | {n_m:.4f} ± {n_s:.4f}     | {note}")
